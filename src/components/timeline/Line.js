...
export default class Timeline extends Component {
      ...
      case 'single-column-right':
        content = (
          <View
            ...
            <View>{this.renderTime(item, index)}</View>
            <View>{this.renderCircleAndLineVertical(item, index)}</View>
            <View style={[{ flex: 1 }]}>{this.renderEvent(item, index)}</View>
          </View>
        );
        break;
      case 'two-column':
        content = (
          <View
            ...
            <View style={{ flex: 1 }}>{this.renderTime(item, index)}</View>
            <View>{this.renderCircleAndLineVertical(item, index)}</View>
            <View style={{ flex: 1 }}>{this.renderEvent(item, index)}</View>
          </View>
        );
  ...
  renderTime = (item, index) => {
    ...
    renderTimeBottom = item.renderTimeBottom
      ? item.renderTimeBottom
      : renderTimeBottom;
    let hourFormat = null,
      amPmFormat = null;
    ...
    return (
      <View
        ...
        <View style={{ flex: 1 }}>{renderTimeBottom()}</View>
      </View>
    );
  };
  ...
  renderEvent = (item, index) => {
    ...
    detailContainerStyle = item.detailContainerStyle
      ? item.detailContainerStyle
  ...
  renderCircleAndLineVertical = (item, index) => {
          ...
          style={[
            {
              ...
              backgroundColor: data[index - 1].lineColor
                ? data[index - 1].lineColor
                : lineColor,
            },
          ]}
        />
      );
    let renderBottomLine = renderLine ? (
      renderLine
    ) : (
    ...
    return (
      <View
        style={{ alignItems: 'center', flex: 1, width: widthLineContainer }}>
        {index != 0 ? renderTopLine : null}
        ...
        <View
          style={[
            ...
            circleStyle,
            {
              ...
              width: circleSize,
              height: circleSize,
              marginTop: spacingDot,
              marginBottom: spacingDot,
            },
            index == 0 ? { marginTop: marginTopCircle } : {},
          ]}>
          {innerCircle}
        </View>
      );
  ...
  renderInnerCircle(item) {
    ...
    innerCircleType = item.innerCircleType
      ? item.innerCircleType
      : innerCircleType;
